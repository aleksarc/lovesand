...
def validate_data(values):
    """
    Inside the try, converts all string bvalues into integers.
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 vallues required, you provided {len(values)}"
            )
...
def calculate_stock_data(data):
    """
    Calculate the average stock fo each item type, adding 10%
    """
